...
module tb_otfs_tx;
    ...
    task automatic pulse_cp_frame_valid();
        begin
            @(posedge clk);
            cp_frame_valid <= 1'b1;
            @(posedge clk);
    ...
    initial begin
        ...
        // Run the IFFT stage using the captured ISFFT frame.
        @(posedge clk);
        ifft_frame_valid <= 1'b1;
        @(posedge clk);
        ifft_frame_valid <= 1'b0;

        // Allow the IFFT output arrays to settle before feeding the CP inserter.
        @(posedge clk);

        pulse_cp_frame_valid();
