...
* **Day 2 (May 19):** Foundational Understanding of The algorithm
    - **Objectives**
        ...
        3. **Domain Transform Orchestration (ISFFT):** Execute the 2D "matrix transform sandwich" $(\mathbf{W}_M​⋅D⋅\mathbf{W}_N^{−1}​)$ to rotate abstract environmental coordinates into standard multi-carrier Time-Frequency coordinates $(\mathbf{X}_{TF}​)$.

        4. **Hardware Stride Optimization Design:** Analyze memory-mapping bottlenecks associated with row-major block writes versus vertical column reads to plan high-throughput, stall-free BRAM transposition architectures.<br/>
    ...
    *   **Energy Conservation:** To guarantee mathematical stability and avoid numeric overflow during fixed-point RTL processing, the forward and inverse transform matrices are scale-normalized by $\frac{1}{\sqrt{M}}$ and $\frac{1}{\sqrt{N}}$ respectively, preserving Parseval's energy invariance: $\|\mathbf{X}_{TF}\|_F^2 = \|\mathbf{D}\|_F^2$.
...
* **Day 4 (May 21, 2026):** End-to-End 16-QAM Floating-Point Reference Model & DAC Waveform Verification
    - **Objectives:**
        1. **Construct Golden 16-QAM Reference Model:** Build and execute a unified Python script tracking the entire pipeline from raw bit ingestion and Gray mapping to 1D continuous waveform synthesis.
        2. **Implement Cyclic Prefix Array Appending:** Integrate the time-domain CP guard interval to mechanically shield back-to-back serialized time slots from multipath echo interference.
        3. **Extract Physical IQ Voltages:** Verify and output raw complex baseband voltages destined for the hardware DAC pins to confirm mathematical simulation bounds.

    ##### 1. System Implementation Architecture
    The reference model maps out the complete transmitter chain. It ingests an input data vector, parses it into 4-bit nibbles, modulates it using a noise-resilient 16-QAM Gray Code assignment table, packs it into the geometric Delay-Doppler matrix ($\mathbf{D}$), passes it through the unitary ISFFT matrix sandwich to yield the Time-Frequency grid ($\mathbf{X}_{TF}$), synthesizes a continuous analog-like trace via sinc interpolation, and appends the final time-domain Cyclic Prefix buffers.

    ```text
    [ Raw Bits ] ──► [ 16-QAM Map ] ──► [ Matrix D ] ──► [ ISFFT Sandwich ] ──► [ Matrix X_TF ] ──► [ Sinc / CP Loop ] ──► [ 1D DAC Voltages ]